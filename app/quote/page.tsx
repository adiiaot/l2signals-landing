  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-6">
      <div className="card text-center">
        <p className="text-[11px] tracking-[0.16em] text-accent-gold uppercase">Quoting • Fixed price where possible</p>
        <h1 className="text-2xl font-bold mt-2">Get a quote — DM the developer</h1>
        <p className="text-sm text-text-muted mt-2 max-w-2xl mx-auto">Every build is scoped. Pick a tier below or send details — we reply within 24h with a fixed price or a short call to scope.</p>
        <div className="flex flex-wrap gap-2 justify-center mt-4">
          <a href="https://t.me/l2signals" target="_blank" rel="noopener" className="btn-secondary rounded-full px-5 py-2 text-xs">Telegram — t.me/l2signals</a>
          <a href="mailto:l2signalslab@gmail.com" className="btn-primary rounded-full px-5 py-2 text-xs">l2signalslab@gmail.com</a>
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-3 gap-3">
        {TIERS.map(t => (
          <div key={t.name} className={`card text-center ${t.highlight ? 'card-glow' : ''}`}>
            ...
            <p className="text-2xl font-bold font-mono mt-1">{t.price}</p>
      ...
      <div id="inquiry" className="card">
        <h2 className="text-base font-bold">Inquiry — tell us about your system</h2>
        <p className="text-xs text-text-muted mt-1">We use this to give you a fixed quote. Or <a href="mailto:l2signalslab@gmail.com" className="text-accent-gold hover:underline">email directly</a>.</p>
        <div className="grid md:grid-cols-2 gap-3 mt-4">
          <label className="text-xs">Name<input value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="AOT" className="mt-1 w-full rounded-lg border px-3 py-2 text-sm bg-surface-overlay" style={{ borderColor: 'var(--glass-border)' }} /></label>
          <label className="text-xs">Email<input value={form.email} onChange={e => setForm({ ...form, email: e.target.value })} placeholder="you@fund.com" className="mt-1 w-full rounded-lg border px-3 py-2 text-sm bg-surface-overlay" style={{ borderColor: 'var(--glass-border)' }} /></label>
          <label className="text-xs">Project type<select value={form.type} onChange={e => setForm({ ...form, type: e.target.value })} className="mt-1 w-full rounded-lg border px-3 py-2 text-sm bg-surface-overlay" style={{ borderColor: 'var(--glass-border)' }}><option>Trading system</option><option>Automation / pipeline</option><option>Dashboard / web app</option><option>AI / LLM tool</option><option>Other</option></select></label>
          <label className="text-xs">Budget<select value={form.budget} onChange={e => setForm({ ...form, budget: e.target.value })} className="mt-1 w-full rounded-lg border px-3 py-2 text-sm bg-surface-overlay" style={{ borderColor: 'var(--glass-border)' }}><option>$500–$2k</option><option>$2.5k–$5k</option><option>$5k–$10k</option><option>$10k+</option><option>Retainer</option></select></label>
          <label className="text-xs md:col-span-2">Message<textarea value={form.message} onChange={e => setForm({ ...form, message: e.target.value })} rows={4} placeholder="What should the system do? Data, markets, users, deadline..." className="mt-1 w-full rounded-lg border px-3 py-2 text-sm bg-surface-overlay" style={{ borderColor: 'var(--glass-border)' }} /></label>
        </div>
        <div className="flex flex-wrap gap-2 mt-4">
          <a href={mailto} onClick={() => setSent(true)} className="btn-primary rounded-full px-6 py-2.5 text-xs">Send via email →</a>
          <a href="https://t.me/l2signals" target="_blank" rel="noopener" className="btn-secondary rounded-full px-6 py-2.5 text-xs">DM on Telegram</a>
        </div>
